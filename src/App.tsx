...
const Typewriter = ({ text, delay = 60 }: { text: string; delay?: number }) => {
  ...
  return <span className="typewriter font-mono text-primary uppercase tracking-widest">{displayedText}</span>;
};

// --- Folder Card Content (Extracted from FolderCard for reuse) ---
const FolderCardContent = ({ project, index }: { project: any, index: number }) => {
  return (
    <div className="w-full h-full flex flex-col">
      {/* Folder Tab */}
      <div className="absolute -top-6 left-0 w-32 h-8 bg-primary/90 rounded-t-lg clip-folder-tab flex items-center px-4">
        <span className="font-mono text-[9px] uppercase tracking-widest text-white font-bold">Project_0{index + 1}</span>
      </div>
      ...
      <div className="w-full h-full bg-[#0a0a0a] rounded-tr-2xl rounded-b-2xl overflow-hidden flex flex-col border border-primary/20 shadow-2xl relative">
        ...
        <div className="h-10 bg-white/5 border-b border-white/5 flex items-center px-6 justify-between relative z-10">
          ...
          <span className="font-mono text-[10px] text-white/40 uppercase tracking-widest">{project.id}.sys</span>
        </div>
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="flex flex-wrap gap-3">
              {project.tags.map((tag: string) => (
                <span key={tag} className="px-3 py-1 border border-white/10 rounded-full font-mono text-[9px] text-primary uppercase tracking-widest">
                  {tag}
                </span>
              ))}
            </div>
...
export default function App() {
  const [, setDimensions] = useState({ width: window.innerWidth, height: window.innerHeight });
    ...
    <div className="relative min-h-screen bg-bg text-white overflow-x-hidden selection:bg-primary selection:text-white">
      ...
      <nav className="fixed top-0 w-full z-50 px-8 py-10 flex justify-end items-center">
        <div className="hidden md:flex gap-12 font-mono text-[10px] uppercase tracking-[0.4em] text-white/40 mr-auto ml-auto">
          {['Home', 'Projects', 'About', 'Expertise', 'Contact'].map((item, i) => (
            <motion.a
              key={item}
              href={`#${item === 'Home' ? 'home' : (item === 'Projects' ? 'archive' : (item === 'About' ? 'profile' : item.toLowerCase()))}`}
        ...
        <motion.button
          ...
          className="glass px-8 py-2.5 rounded-full text-[10px] uppercase tracking-widest font-bold hover:bg-primary/10 transition-all border-primary/20"
        >
          Hire_Me
        </motion.button>
      </nav>
      ...
      {/* Hero Content */}
      <section id="home" className="relative h-screen flex flex-col justify-center items-center px-6 z-10 pointer-events-none">
        ...
        <div className="absolute bottom-12 left-8 right-8 flex justify-end items-end">
          ...
          >
            <div className="flex gap-8">
              <a href="https://github.com/jlcoding01" target="_blank" rel="noopener noreferrer">
                <Github className="w-5 h-5 cursor-pointer hover:text-primary transition-colors" />
              </a>
              <a href="https://www.linkedin.com/in/jerryliuintech/" target="_blank" rel="noopener noreferrer">
                <Linkedin className="w-5 h-5 cursor-pointer hover:text-primary transition-colors" />
              </a>
            </div>
            <div className="flex flex-col items-end">
              <span className="font-mono text-[9px] uppercase tracking-[0.4em] text-white/20 mb-1">Scroll_to_Decrypt</span>
      ...
      <section id="archive" className="relative z-10 py-40">
        <div className="max-w-7xl mx-auto mb-20 px-8 w-full">
          <span className="font-mono text-primary text-xs uppercase tracking-[0.5em] mb-4 block">01</span>
        ...
        >
          {projects.map((project, i) => (
            <ScrollStackItem key={project.id} itemClassName="!bg-transparent !border-none !shadow-none !p-0 !min-h-0 aspect-auto md:aspect-[16/10] min-h-[600px] md:min-h-0">
        ...
        <div className="max-w-7xl mx-auto px-8">
          <div className="flex flex-col lg:flex-row items-center gap-20">
            ...
            <div className="w-full lg:flex-1 space-y-8">
              <span className="font-mono text-primary text-xs uppercase tracking-[0.5em] block">02</span>
              <h2 className="text-5xl md:text-8xl font-black uppercase tracking-tighter leading-none">
                About
              </h2>
              <p className="font-mono text-sm text-white/40 leading-loose max-w-xl uppercase tracking-widest">
                I bring ideas to life with efficient code and innovative solutions. 
                Beyond coding, I embrace the mindset of an Entrepreneur, constantly exploring new opportunities to create impactful products.
                Let's build something extraordinary together.
              </p>
              <div className="grid grid-cols-2 gap-8 pt-8">
                <div>
                  <div className="text-primary font-mono text-2xl font-bold mb-2">05+</div>
                  <div className="text-white/20 font-mono text-[10px] uppercase tracking-widest">Years_Exp</div>
                </div>
                <div>
                  <div className="text-primary font-mono text-2xl font-bold mb-2">42+</div>
                  <div className="text-white/20 font-mono text-[10px] uppercase tracking-widest">Deployments</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section id="modules" className="relative z-10 px-8 py-40">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
            <div>
              <span className="font-mono text-primary text-xs uppercase tracking-[0.5em] mb-4 block">03</span>
              <h2 className="text-5xl md:text-7xl font-black uppercase tracking-tighter">Expertise</h2>
            </div>
            <p className="font-mono text-[10px] text-white/30 uppercase tracking-widest max-w-xs text-right">
              Specialized in low-level visual engineering and high-performance interface architecture.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { title: 'Frontend', icon: Cpu, desc: 'Building responsive, accessible, and high-performance UIs with React and Next.js.' },
              { title: 'Backend', icon: Layers, desc: 'Designing scalable microservices, RESTful APIs, and robust database schemas.' },
              { title: 'Cloud_DevOps', icon: Shield, desc: 'Deploying resilient infrastructure on AWS/GCP with Docker and Kubernetes.' }
            ].map((item, i) => (
              <motion.div
                key={i}
              ...
              >
                <div className="absolute top-0 right-0 p-4 opacity-10 font-mono text-4xl font-black">0{i+1}</div>
                <item.icon className="w-12 h-12 text-primary mb-10 group-hover:scale-110 transition-transform duration-500" />
                <h3 className="font-mono text-xl font-bold mb-6 uppercase tracking-tight">{item.title}</h3>
                <p className="font-mono text-[11px] text-white/40 leading-relaxed uppercase tracking-widest">{item.desc}</p>
                <div className="mt-12 flex justify-between items-center">
                  <div className="flex gap-1">
                    {[1, 2, 3].map(b => <div key={b} className="w-4 h-1 bg-primary/20 group-hover:bg-primary/60 transition-colors" />)}
                  </div>
      ...
      {/* Contact Section */}
      <section id="contact" className="relative z-10 py-40 px-8 border-t border-white/5">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="flex flex-wrap justify-center gap-12">
            {[
              ...
              { name: 'Linkedin', url: 'https://www.linkedin.com/in/jerryliuintech/' },
