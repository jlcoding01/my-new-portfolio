...
export const ScrollStackItem: React.FC<{ children: React.ReactNode; itemClassName?: string }> = ({ 
  children, 
  itemClassName = '' 
}) => (
...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    // We need the end element's natural position
    // If it's being transformed, this is tricky. 
    // But usually the end element is just a spacer at the bottom.
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      if (blurAmount) {
        ...
        for (let j = 0; j < cardsRef.current.length; j++) {
          const jCardTop = initialOffsetsRef.current[j];
